"""
pipelines/huey_ling/batch/run_batch.py
──────────────────────────────────────
Batch pipeline runner for Huey Ling (Seller Performance).

Usage:
    python pipelines/huey_ling/batch/run_batch.py               # all steps, full refresh
    python pipelines/huey_ling/batch/run_batch.py --mode cdc    # all steps, incremental
    python pipelines/huey_ling/batch/run_batch.py --step meltano
    python pipelines/huey_ling/batch/run_batch.py --step gcs-to-bq
    python pipelines/huey_ling/batch/run_batch.py --step dbt [--mode full|cdc]
...
_CONFIG_PATH = Path("dashboards/huey_ling/config/gcp_config.yaml")
...
def _read_gcp_config() -> dict:
    ...
    if not _CONFIG_PATH.exists():
        raise FileNotFoundError(
            ...
            "  Run: cp config/gcp_config_template.yaml dashboards/huey_ling/config/gcp_config.yaml"
        )
...
def run_meltano():
    ...
    meltano_dir = "pipelines/huey_ling/batch/meltano"
    try:
        ...
        log("  Run: pip install meltano  &&  cd pipelines/huey_ling/batch/meltano && meltano install")
        sys.exit(1)
...
def run_dbt(mode: str = "full"):
    ...
    # Resolve keyfile: prefer dashboards/huey_ling/config/service_account.json,
    # fall back to the key path in gcp_config.yaml (or profiles.yml default).
    project_root = Path(__file__).resolve().parent.parent.parent.parent
    candidate = project_root / "dashboards" / "huey_ling" / "config" / "service_account.json"
    if not candidate.exists():
        try:
            candidate = Path(_read_gcp_config().get("key_path", ""))
        except Exception:
            candidate = Path("")
    env = {**os.environ}
    if candidate.exists():
        env["DBT_KEYFILE"] = str(candidate)
    try:
        result = subprocess.run(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Olist batch pipeline runner — Huey Ling")
    parser.add_argument(
        "--step",
    ...
    log(f"=== Huey Ling Batch Pipeline Start (step={args.step}, mode={args.mode}) ===")
    start = time.time()
    ...
    log(f"=== Huey Ling Batch Pipeline Complete ({elapsed}s) ===")
